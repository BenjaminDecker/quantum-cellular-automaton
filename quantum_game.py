...
def recursive_big_n_calculator(index, offset, alive_count):
    ...
    dead = np.dot(dead_small_n_operators[(
        index + offset) % NUM_CELLS], recursive_big_n_calculator(index, offset + 1, alive_count))
    alive = np.dot(alive_small_n_operators[(
        index + offset) % NUM_CELLS], recursive_big_n_calculator(index, offset + 1, alive_count + 1))
    return dead + alive
...
for i in STEP_RANGE:
    if PERIODIC_BOUNDARIES:
        ...
        print("Step " + str(i - DISTANCE + 1) +
              " of " + str(NUM_CELLS - 2 * DISTANCE))
    ...
    for i in range(NUM_STEPS):
        for j in range(NUM_CELLS):
            ...
            partial_trace = np.trace(density_matrix.reshape(
                2, 2**(NUM_CELLS - 1), 2, 2**(NUM_CELLS - 1)), axis1=1, axis2=3)
