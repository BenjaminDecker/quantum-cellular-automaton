...
\begin{frame}[t]{What do we need?}
    ...
    \vspace{15pt}
    \begin{center}
        For an initial state $\ket{\psi}_0$ and a unitary time evolution operator $\hat{U}(k)$, the state after $k$ time steps is given by
    \end{center}
    \vspace{10pt}
    \begin{equation*}
...
\begin{frame}[t]{Iterative Calculation}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=\textwidth]{graphics/not_interesting_f12.png}
        \caption{Time evolution of the initial state $\ket{\psi}_0=\ket{0}^{\otimes3}\otimes\ket{1001}\otimes\ket{0}^{\otimes4}$ according to equation (\ref{eq:singleTimeStepEvolution}) shown as the distribution of alive cells}
        \label{fig:notinteresting}
    \end{figure}
    }%
    \only<3>{
    \begin{figure}
...
\begin{frame}[t]{Rule 150}
    ...
    \end{equation}
    \begin{center}
        "A cell is flipped if the number of alive cells among its nearest neighbors is $1$"
    \end{center}
    \only<2>{
    \begin{figure}
    \begin{columns}
...
\begin{frame}[noframenumbering]{Desciption for plots in slide "The Quantum Game of Life"}
    \only<1>{
    \begin{itemize}
        \item Classical: The time evolution corresponding to a classical cellular automaton
        \item Quantum: The time evolution of the distribution of alive cells
        \item Rounded: The rounded version of "Quantum"
        \item single site entropy: The time evolution of the single site entropy
    \end{itemize}
    }
    \only<2>{
    \begin{center}
    ...
    \end{center}
    \begin{itemize}
        \item rule: $f_{12}$, $t=\frac{\pi}{2}$, $\ket{\psi}_0 = \ket{0}^{\otimes 4}\otimes \ket{101}\otimes \ket{0}^{\otimes 4}$, start time step $k=0$
        \item rule: $150$, $t=\frac{\pi}{2}$, $\ket{\psi}_0 = \ket{0}^{\otimes 4}\otimes \ket{101}\otimes \ket{0}^{\otimes 4}$, start time step $k=0$
        \item rule: $150$, $t=\frac{\pi}{2}$, $\ket{\psi}_0 = \ket{0}^{\otimes 5}\otimes \ket{1}\otimes \ket{0}^{\otimes 5}$, start time step $k=0$
        \item rule: $150$, $t=\frac{\pi}{2}$, $\ket{\psi}_0 = \ket{0}^{\otimes 3}\otimes \ket{10101}\otimes \ket{0}^{\otimes 3}$, start time step $k=0$
        \item rule: $150$, $t=\frac{\pi}{2}$, $\ket{\psi}_0 = \ket{0}\otimes \ket{1}^{\otimes 9}\otimes \ket{0}$, start time step $k=0$
        \item rule: $150$, $t=\frac{\pi}{2}$, $\ket{\psi}_0=\bigotimes_{k = 0}^{N - 1}\left[R_x\left(\pi\frac{k}{N - 1}\right)\ket{1}\right]$, start time step $k=0$
        \item rule: $150$, $t=\frac{\pi}{10}$, $\ket{\psi}_0=\bigotimes_{k = 0}^{N - 1}\left[R_x\left(\pi\frac{k}{N - 1}\right)\ket{1}\right]$, start time step $k=0$
        \item rule: $150$, $t=\frac{\pi}{2}$, $\ket{\psi}_0=\bigotimes_{k = 0}^{N - 1}\left[R_x\left(\pi\frac{k}{N - 1}\right)\ket{1}\right]$, start time step $k=70$
        \item rule: $150$, $t=\frac{\pi}{10}$, $\ket{\psi}_0=\bigotimes_{k = 0}^{N - 1}\left[R_x\left(\pi\frac{k}{N - 1}\right)\ket{1}\right]$, start time step $k=70$
    \end{itemize}
    }
\end{frame}

\begin{frame}[noframenumbering]{Wolfram Code}
